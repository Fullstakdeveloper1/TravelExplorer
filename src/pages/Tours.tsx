...
function Tours() {
  ...
  const {
    ...
    loading: tourLoad,
    error: tourErr,
  } = useFetch<TourType[]>(toursService);
  const {
    data: categories,
    loading: catLoad,
    error: catErr,
  } = useFetch<CategoryType[]>(categoriesService);
          ...
          <div className="items">
            ...
            {categoriesData?.map((category) => (
              <div
                ...
                key={category.id}
                className={`name ${
                  active == String(category.id) ? "active" : ""
                }`}
              >
                {category.name}
              </div>
          ...
          <div className="tours">
            {filteredTour.map((tour) => (
              <div key={tour.id} className="tour">
                ...
                <div className="info">
                  <div className="head">
                    <div className="rating">{calculateRating(tour.rating)}</div>
                    <div className="price">{tour.price}</div>
                  </div>
                  <div className="pack-includes">
                    {Array.isArray(tour.pack_includes) ? (
                      tour.pack_includes.map((pack, index) => (
                        <li key={index}>✅ {pack}</li>
                      ))
                    ) : (
                      <li>✅ {tour.pack_includes}</li>
                    )}
                  </div>
                  <button onClick={() => navigate(`/tour/${tour.id}`)}>
                    Read More
                  </button>
                </div>
              </div>
